...
class Hcaptcha extends BaseCaptcha
{
    ...
    {
        $key = FormBuilder::getInstance()->getSettings()->captchas['hcaptcha']['siteKey'];
        return App::parseEnv($key);
    }

    public function getSecretKey(): string
    {
        $key = FormBuilder::getInstance()->getSettings()->captchas['hcaptcha']['secretKey'];
        return App::parseEnv($key);
    }

    private function _getErrorMessage(): string
    {
        return FormBuilder::getInstance()->getSettings()->captchas['hcaptcha']['errorMessage'];
    }
